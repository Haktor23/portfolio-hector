    ...
    <nav className={`fixed top-0 w-full z-40 transition-all duration-300 ${isScrolled ? 'py-4' : 'py-6'}`}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex justify-center">
        <div className={`glass-card rounded-full px-6 py-3 flex items-center gap-6 transition-all ${isScrolled ? 'bg-black/80' : 'bg-transparent border-transparent'}`}>
          <a href="#bento" className="hover:text-accent transition-colors text-sm font-medium">{t.nav.home}</a>
          <a href="#work" className="hover:text-accent transition-colors text-sm font-medium">{t.nav.work}</a>
          <a href="#about" className="hover:text-accent transition-colors text-sm font-medium">{t.nav.about}</a>
          <a href="#contact" className="hover:text-accent transition-colors text-sm font-medium">{t.nav.contact}</a>
